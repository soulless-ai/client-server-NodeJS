...
router.post('/api_load_csv', async function (req, res) {
  ...
  const fileUrl = decodeURIComponent(req.body.fileUrl);
  try {
    // Проверка наличия email и токена в missions
    if (userId === null) {
      res.json({ success: false, message: 'Запрос не авторизован. Пожалуйста, авторизуйтесь.' });
    } else {
      const queryTask = 'INSERT INTO tasks (date_set, id_client, id_account, id_services, operation, file) VALUES (now(), $1, $2, $3, $4, $5) RETURNING task_number';
      const valuesTask = [userId, account_id, service_id, task, fileUrl];
